    ...
    (() => {
        ...
        function initSliders() {
            if (document.querySelector(".hero-home__slider")) new core(".hero-home__slider", {
                ...
                pagination: {
                    el: ".hero-home__swiper-pagination.swiper-pagination",
                    clickable: true
                },
                on: {}
            });
            if (document.querySelector(".interested__slider")) new core(".interested__slider", {
                ...
                loop: true,
                pagination: {
                    el: ".interested__swiper-pagination.swiper-pagination",
                    clickable: true
                },
                breakpoints: {
                    320: {
            ...
            if (document.querySelector(".related__slider")) new core(".related__slider", {
                ...
                loop: true,
                pagination: {
                    el: ".related__swiper-pagination.swiper-pagination",
                    clickable: true
                },
                breakpoints: {
                    320: {
            ...
            if (document.querySelector(".mini-images-view")) {
                const myThumbsSwiper = new core(".mini-images-view", {
                ...
                new core(".images-product-view__slider", {
                    modules: [ Autoplay, Thumb ],
                    observer: true,
                    observeParents: true,
                    slidesPerView: 1,
        ...
        window.onload = function() {
            ...
            function documentActions(e) {
                ...
                if (targetElement.classList.contains("search-form__icon")) {
                    ...
                    icon.classList.add("_display-none");
                } else if (!targetElement.closest(".search-form") && document.querySelector(".search-form._active")) {
                    document.querySelector(".search-form").classList.remove("_active");
                    icon.classList.remove("_display-none");
                }
                if (targetElement.closest(".block-actions__favorite")) {
                    const iconsFavorite = document.querySelectorAll(".block-actions__favorite");
                    iconsFavorite.forEach((function(icon) {
                        targetElement.classList.toggle("_active-red");
                    }));
                }
                if (targetElement.closest(".block-actions__search")) {
                    const iconsScale = document.querySelectorAll(".block-actions__search");
                    iconsScale.forEach((function(icon) {
                        icon.classList.remove("_active-scale");
                        targetElement.classList.add("_active-scale");
                        if (icon.classList.contains("_active-scale")) {
                            const carrentImageBox = targetElement.closest(".all-plants__image-box");
                            const carrentImage = carrentImageBox.querySelector(".all-plants__image");
                            carrentImage.classList.toggle("_to-scale");
                        }
                    }));
                }
                if (targetElement.closest(".product-scale-icon")) {
                    const iconsScale = document.querySelectorAll(".product-scale-icon");
                    iconsScale.forEach((function(icon) {
                        icon.classList.remove("_active-scale");
                        targetElement.classList.add("_active-scale");
                        if (icon.classList.contains("_active-scale")) {
                            const carrentSlide = targetElement.closest(".images-product-view__slide-ibg_contain");
                            const carrentImage = carrentSlide.querySelector("img");
                            console.log(carrentImage);
                            carrentImage.classList.toggle("_to-scale");
                        }
                    }));
                }
                if (targetElement.closest(".button-delete")) {
                    const item = targetElement.closest(".table__row");
                ...
                if (targetElement.classList.contains("actions-product__button")) {
                    const card = targetElement.closest(".item-product");
                    const productInfo = {
                        id: card.dataset.pid,
                        imgSrc: card.querySelector(".item-product__image-image").getAttribute("src"),
                        title: card.querySelector(".item-product__title").innerText,
                        price: card.querySelector(".item-product__price").innerText,
                        total: card.querySelector(".item-product__price").innerText
                    };
                    localStorage.setItem("productInfo", JSON.stringify(productInfo));
                    const productLS = JSON.parse(localStorage.getItem("productInfo"));
                    console.log(typeof productLS);
                }
                if (targetElement.closest(".quantity")) {
                    const rowItem = targetElement.closest(".table__row");
                    let value = targetElement.closest(".quantity").querySelector("input").value;
                    let price = rowItem.querySelector(".table__cell_price").innerText;
                    price = price.replace("$", "");
                    let total = (price * value).toFixed(2);
                    total = `$${total}`;
                    rowItem.querySelector(".table__cell_total").innerHTML = `${total}<button type="button" class="button-delete"></button>`;
                }
            }
            let tabsAcc = document.getElementsByClassName("left-aside__text");
            let urlAcc = document.location.href;
            for (let i = 0; i < tabsAcc.length; i++) if (urlAcc == tabsAcc[i].href) tabsAcc[i].classList.add("_active");
        };
        window.addEventListener("storage", (function(e) {
            console.log("change");
            const productINLS = JSON.parse(localStorage.getItem("productInfo"));
            const table = document.querySelector(".table__body");
            if (table) {
                const cartItemHtml = `\n\t\t           <tr data-card-pid="${productINLS.id}" class="table__row">\n            <td class="table__cell table__cell_big">\n              <div class="line-table__image-box">\n                <div class="line-table__image -ibg"><img src="${productINLS.imgSrc}" alt="${productINLS.title}" /></div>\n              </div>\n              <div class="line-table__info">\n                <div class="line-table__name">${productINLS.title}</div>\n              </div>\n            </td>\n            <td class="table__cell table__cell_price">${productINLS.price}</td>\n            <td class="table__cell table__cell_quantity">\n              <div class="quantity-wrapper">\n                <div class="quantity">\n                <button data-action="minus" type="button" class="quantity__button quantity__button_minus"></button>\n                  <div class="quantity__input">\n                    <input autocomplete="off" type="text" name="form[]" value="1" />\n                  </div>\n\t\t\t\t\t\t <button data-action="plus" type="button" class="quantity__button quantity__button_plus"></button>\n                </div>\n              </div>\n            </td>\n   \t\t <td class="table__cell table__cell_total">${productINLS.price}<button type="button" class="button-delete"></button></td>\n          </tr>\n\t\t  `;
                table.insertAdjacentHTML("beforeEnd", cartItemHtml);
            }
        }));
        window["FLS"] = true;
        isWebp();
